...
- Go-live remains: GitHub Releases for the five `sdd-mcp` binaries, pack copy, admin-portal sync. Those are not open Sprint 2 SBIs.
- [MCP-01](./product-backlog.md#pb-16) stays ToDo for that go-live slice. Sprint 2 installer stories are Done.
...
| feature-04 | [Web-portal-08 Instructions page — Get secret](./product-backlog.md#pb-74)                                                       | Web-portal | Feature | Secret form at the bottom of Features   | - The Features tab shows the field and button after the template list. - The hint is “Enter the name of the secret, example: sdd-trial-googlemaps”, “输入要获得的密钥名称，例如：sdd-trial-googlemaps”, and “輸入要取得的密鑰名稱，例如：sdd-trial-googlemaps”. - The button is “Get secret”, “获取密钥”, and “獲取密鑰”. - The Setup tab does not show the form.                                                                                                                                                                                                   | `[app-stories.md](./admin-portal/app-stories.md)` AC7 · `[app-design.md](./admin-portal/app-design.md)` `/instructions` · `[app-test.md](./admin-portal/app-test.md)` · `[13-instructions.html](./admin-portal/ui-mockup/13-instructions.html)` | - Strings are i18n keys `admin.guide.secret_hint` and `admin.guide.secret_button`. - Lookup is feature-05. - Showing a value is feature-06.                                                                                                                                                                                                                     | Done   |
| feature-05 | [Web-portal-08 Instructions page — Get secret](./product-backlog.md#pb-74)                                                       | Web-portal | Feature | Server returns one secret by exact name | - A stored name returns only that plaintext value. - An unknown name is not found. - The response lists no other names or values. - The lookup reads the admin key store on the server.                                                                                                                                                                                                                                                                                                                     | `[mcp/mcp-stories.md](./mcp/mcp-stories.md)` `sdd-mcp-get-key` · `src/core/tools/get-key.ts`                                                 | - Does not add a token product. - Does not register `sdd_get_key` on stdio.                                                                                                                                                                  | Done   |
| feature-06 | [Web-portal-08 Instructions page — Get secret](./product-backlog.md#pb-74)                                                       | Web-portal | Feature | Button shows the value or not-found     | - Submit stays on Features at `#features-secret` (`?tab=features`). - After a result, `secret-result` or `secret-error` is scrolled into view. - A known exact name shows that plaintext in a code block with a copy control, width equal to the secret-lookup row. - An unknown name shows `admin.guide.secret_missing` at the same width and no other values. - An empty name shows `admin.guide.secret_empty` and does not call the lookup API. - The setup paste sentence and `mcp.json` sample stay without a token.                                                                                                                                                                                                                                                                                           | `[product-backlog.md](./product-backlog.md#pb-74)` · `src/components/features/InstructionsPage.tsx` · [`issues-log.md`](./issues-log.md) WA-09 · WA-11 | - Depends on feature-04 for the form and feature-05 for the lookup. - Start only after feature-10 (Web-portal-11) is Done. - WA-11 layout is a follow-up on this row.                                                                                                                                                                          | Done   |
