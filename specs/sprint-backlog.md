...
| feature-16 | [Web-portal-12 Instructions tab: sdd-scrum guide](./product-backlog.md#pb-81) | Web-portal | Feature | Guide tab on instructions | - `/` and `/instructions` show a third tab for the sdd-scrum guide. - The tab label is an i18n key in `en`, `zh-Hans`, and `zh-Hant`. - The panel shows the guide for the active locale. - A missing locale file shows the English guide. - Setup and Features still switch. - Get secret is not on the guide tab. | [`sdd-scrum-guide.md`](./framework.seeds/templates/EN/sdd-scrum-guide.md) · [`app-design.md`](./admin-portal/app-design.md) | - Not started. - Does not replace the Features catalog. - After feature-17, Get secret is on Setup. | ToDo |
| feature-17 | [Web-portal-13 Get secret moves to Setup](./product-backlog.md#pb-83) | Web-portal | Feature | Move Get secret to Setup | - On `/` and `/instructions`, Setup shows the secret form after the tools table. - Features does not contain `secret-lookup`. - The anchor is `#setup-secret`. - Submit stays on Setup and does not set `?tab=features`. - Hint, button, found value, not-found, empty name, scroll, and lookup-row width stay as feature-05. - The setup sentence and `mcp.json` sample stay without a token. | [ADR-067](./adr/ADR-067-get-secret-on-setup.md) · [`app-design.md`](./admin-portal/app-design.md) · [`13-instructions.html`](./admin-portal/ui-mockup/13-instructions.html) | - Mockup confirmed. - Page matches mockup. - Unit and Playwright green. - Awaiting usability confirmation. | WIP |
