...
| Web-portal | <a id="pb-81"></a>Web-portal-12 | Instructions tab: sdd-scrum guide | `/` and `/instructions` gain a third tab, sdd-scrum guide, beside Setup and Features. The tab shows the guide for the active locale. Tab labels are i18n keys. Setup and Features stay as they are. | A reader opens the guide tab on `/` and on `/instructions`. The panel shows the guide. A missing locale file shows the English guide. Setup and Features still switch. | [Spec-seeds-02](#pb-33) · [`admin-portal/app-design.md`](./admin-portal/app-design.md) `/instructions` | Sprint 3 | ToDo |
| Web-portal | <a id="pb-83"></a>Web-portal-13 | Get secret moves to Setup | The secret form leaves the bottom of Features and sits at the bottom of Setup, after the tools table. Features does not show the form. Lookup, copy, not-found, and empty-name behavior stay as [Web-portal-08](#pb-74). | On `/` and `/instructions`, Setup shows `secret-lookup` after the tools table. Features does not contain it. Submit stays on Setup at `#setup-secret` and does not set `?tab=features`. A result still scrolls into view at the lookup-row width. | [ADR-067](./adr/ADR-067-get-secret-on-setup.md) · [Web-portal-08](#pb-74) · [`admin-portal/ui-mockup/13-instructions.html`](./admin-portal/ui-mockup/13-instructions.html) | Sprint 3 | WIP |
