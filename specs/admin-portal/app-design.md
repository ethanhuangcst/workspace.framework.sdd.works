...
| Guide tab (Web-portal-12 / feature-16) | Not built yet. When built, a third tab beside Setup and Features shows [`scrum-in-sdd.md`](../framework.seeds/templates/EN/scrum-in-sdd.md) for the active locale (`en` → EN, `zh-Hans` → HanS, `zh-Hant` → HanT), with English fallback when that locale file is missing. Tab labels are i18n keys. Get secret stays on Setup ([ADR-067](../adr/ADR-067-get-secret-on-setup.md)). Filename: [ADR-068](../adr/ADR-068-scrum-in-sdd-filename.md). |
